...
| M-18 AI 商品搜尋推薦 | ✅ | ✅ | ✅ | ✅ | 🔵 | ✅ | PR #62 的公開 Endpoint、額度、SQL 候選、組裝／相容性、Fail Closed、關鍵字降級、UI 與契約已合併。未合併工作樹後續收束為單次 5 秒 AI 意圖呼叫、後端核准事實確定性理由與低延遲設定；DEC-BATCH-055 再以 `product-search-v5` 定版一般「主機」taxonomy、避免由「遊戲美術」職稱推論 Gaming，並加入只含固定原因碼＋欄位名稱的內部 InvalidOutput 診斷。公開 API／資料庫不變；Infrastructure 24／24、Application 10／10、14 筆 deterministic-only orchestration 14／14、無結果 UI 7／7 均通過。歷史 v4 低延遲 Smoke 的商品 P95 為 3,013 ms，但正式覆核 3 Pass／3 Fail；v5 尚待另行授權付費重驗，Live Gate 仍由 AI-09 追蹤。 |
...
- AI 客服已有歷史 Live 樣本；商品搜尋 v4 低延遲 Smoke 已執行但品質失敗。`product-search-v5` 的 Live Model 品質、P95、Token 與成本尚未重驗；由 AI-09 獨立追蹤，不回退 M-18／M-19 已合併狀態。
