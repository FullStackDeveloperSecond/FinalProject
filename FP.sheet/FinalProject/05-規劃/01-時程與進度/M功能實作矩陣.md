...
| M-02 訪客結帳與訂單驗證 | ✅ | 🟡 | 🟡 | ✅ | ⬜ | 🔵 | PR #52 已將 Checkout SQL Gateway、原子建單、缺貨回滾、整數金額與通知排程基礎合併至 `dev`；仍無 Checkout API／前端／付款成功回呼。訪客查單 PR #40、取消／退貨入口 PR #43 尚未進 `dev`。 |
...
| M-17 零件相容性 | ✅ | 🟡 | 🟡 | ✅ | ⬜ | 🔵 | PR #52 已合併受保護語意鍵、官方 CPU／晶片組映射、來源型 Catalog Reader、多值規格 Migration 與 SQL 證據；完整功能仍與 M-16 共用 PR #34／#35，尚待同步最新 `dev`、修復 CI 及前端／E2E。 |
...
3. E2E 欄只認可從 UI 經 API 到資料庫／Provider 的可重跑旅程。PR #52 已建立 Playwright／CI 基礎及商品瀏覽、匿名管理員登入 Smoke；這不等於核心交易與其他 M 功能 E2E 已完成。
