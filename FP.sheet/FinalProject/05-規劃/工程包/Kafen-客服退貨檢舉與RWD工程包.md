...
環境、SQL Server `.\SQL2025`／`DoSelectDb`、既有 `InitialCreate`、啟動與檢查完全依 `FP.dev/README.md` 與 [[03-架構/本機開發環境與版本基線]]。不要建立新資料庫設計或每模組 Migration。
...
3. [[02-領域需求/客服與AI功能]]、[[02-領域需求/退貨與退款政策]]、[[02-領域需求/評價收藏檢舉與模擬發票規格]]、[[02-領域需求/商品組裝客服與報表驗收規格]]。
4. [[03-架構/資料表實作交付/Kafen-客服售後與檢舉最終Schema]]。

案件查詢另讀 [[03-架構/統一案件工作台設計]]；附件另讀 [[03-架構/檔案與圖片儲存設計]]；SLA 定義可先讀 [[知識點/SLA]]。RWD／品牌不得改變已定 Route、授權與資料契約。
...
前台 Page ID：`C-19`～`C-20`、`C-26`、`C-28`～`C-30`；`C-27` AI 頁由 alex 主責、你提供案例與領域覆核。後台：`A-19`～`A-20`、`A-24`～`A-26`。完整規格見 [[03-架構/M功能桌面UI與Route規格]]。
...
修改 Vue 時，在對應 App 執行 `npm run typecheck`、`npm run lint -- --max-warnings 0`、`npm test`、`npm run build`、`npm audit --omit=dev`。S-07 啟動後另依 [[03-架構/多語系與RWD驗收矩陣]] 驗證 360／768。

## 10. PR、日誌與停止條件

- 遵守 [[Git協作規範]]；每個切片獨立 PR，由 alex 核准並 Squash Merge。
- 依 [[日誌/README]] 記錄 Query／DTO、狀態、SLA 時間、附件授權、跨模組資料取得與測試帳號角色，不記錄真實個資或實體檔案路徑。
- Typed Client 依 [[03-架構/OpenAPI與前端Client流程]] 由正式 OpenAPI 產生，不手寫第二套 DTO。
