...
- 工作台固定投影 12 個共通欄位，不含 CustomerReplyState、RowVersion 或另一套 AssignmentState；授權條件套在每個 UNION 分支，使用 `LastActivityAtUtc DESC, CasePublicId DESC` Cursor 分頁；完整設計見 [[03-架構/07-領域設計/統一案件工作台設計]]。
...
完整策略與審核清單見 [[03-架構/03-資料與一致性/資料庫正規化與反正規化策略]]。
