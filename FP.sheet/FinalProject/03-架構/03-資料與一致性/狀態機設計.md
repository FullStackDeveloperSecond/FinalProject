...
本文件把訂單、付款、退款、組裝、物流、退貨、庫存保留、客服、評價與檢舉拆成獨立狀態機或明確狀態邊界。主要狀態與轉移及客服案件取消條件均已確認。
...
`CouponStatus` 只描述優惠券規則的生命週期；實際套用時仍須驗證期間、門檻、範圍、總名額、每人次數及會員資格。

| 目前狀態 | 操作／事件 | 下一狀態 | 必要條件 |
|---|---|---|---|
| `Draft` | 排定未來生效 | `Scheduled` | 開始時間晚於目前時間且結束時間合法 |
| `Draft` | 立即啟用 | `Active` | 已進入有效期間且規則完整 |
| `Draft` | 永久停用 | `Disabled` | 保存操作者、原因與稽核 |
| `Scheduled` | 到達開始時間 | `Active` | 尚未到期且名額可用 |
| `Scheduled` | 到達結束時間 | `Expired` | 背景工作冪等處理 |
| `Scheduled` | 永久停用 | `Disabled` | 保存操作者、原因與稽核 |
| `Active` | 暫停 | `Paused` | 暫時停止使用，不改變有效期間 |
| `Active` | 名額耗盡 | `Exhausted` | 總使用量達上限 |
| `Active` | 到達結束時間 | `Expired` | 背景工作冪等處理 |
| `Active` | 永久停用 | `Disabled` | 保存操作者、原因與稽核 |
| `Paused` | 恢復 | `Active` | 尚在有效期間且名額可用 |
| `Paused` | 到達結束時間 | `Expired` | 背景工作冪等處理 |
| `Paused` | 永久停用 | `Disabled` | 保存操作者、原因與稽核 |
| `Exhausted` | 名額返還 | `Active` | 使用量重新低於上限、尚未到期且未停用 |
| `Exhausted` | 到達結束時間 | `Expired` | 返還名額不得延長期限 |
| `Exhausted` | 永久停用 | `Disabled` | 保存操作者、原因與稽核 |

- `Expired`、`Disabled` 是終態，不得重新啟用；暫時停止必須使用 `Paused`。
- `activate` 只接受 `Draft` 或符合條件的 `Paused`；`pause` 只接受 `Active`；`disable` 接受所有非終態。非法轉移回傳狀態衝突，不直接覆寫欄位。
- `Scheduled` 只代表尚未到開始時間；若排程執行時已超過結束時間，直接進入 `Expired`，不得短暫轉為 `Active`。
...
- 依 DEC-P344～P346，Demo 付款完成只由既有模擬完成 Endpoint 進入付款完成行為；Controller 不直接轉移 Entity。Service 在同一 SQL Server 交易驗證 Attempt／Order／金額／期限／狀態，新增唯一 PaymentEvent、更新 PaymentAttempt、呼叫 `Order.ApplyPaymentProjection`、寫入 `PaidAmount`／`PaidAtUtc`／狀態歷程與 Outbox。通知與模擬發票只由提交後 Outbox 接續，Replay 不得重複副作用。
- 產品完成 Endpoint 只在 Demo Profile 且 `Demo:SimulationEndpointsEnabled=true` 開放，需 Owner Member／有效 Guest Scope與 Antiforgery；`simulationKey` 同時作為此命令的唯一重播鍵與 Provider Event 鍵，不另要求 `Idempotency-Key` Header。依 DEC-P356，隔離 E2E Environment 可用相同顯式開關測試此寫入入口；Development、Production 與其他產品環境不得提供。
- 即時付款的保留期限最長 15 分鐘；ATM 與超商代碼最長 3 天。實際 `InstructionExpiresAtUtc` 取付款方式期限與訂單原 `PaymentDueAtUtc` 較早者，不得藉由重試延長訂單期限。
...
- 每台工作歷程固定保存於 append-only `AssemblyJobStatusHistories`，以 `AssemblyJobId` 關聯；狀態更新與歷程新增同交易。`OrderStatusHistories` 只保存 `Orders.AssemblyStatus` 聚合投影的變更，不混入單台工作細節。
- `NotRequired` 只允許出現在沒有組裝工作的 `Orders.AssemblyStatus`；實際 `AssemblyJobs.Status` 不含此值。
...
- 每筆有效保留只能成功消耗或釋放一次。
- Reservation 只在 Checkout 建立 Order 後，以 `OrderId` 於同一 SQL Transaction 建立；Cart 不建立 Reservation，也不保存 `CartId`。
...
- 主管角色為 `CustomerServiceSupervisor`；客服與一般檢舉使用不同佇列，但 `CustomerService` 均可自領；退貨不進客服自領佇列。
...
## 9. 商品評價狀態 `ProductReviewStatus`

```text
Draft → PendingReview → Approved | Rejected
Approved → Hidden
Approved --編輯建立 Revision→ PendingReview
Rejected → PendingReview
```

- 只有 `Approved` 可公開；不得使用舊的 `Pending/Published` 第二套列舉。
- 已核准內容編輯時建立 Revision，舊版停止公開，新版回到 `PendingReview`。
- 拒絕、隱藏與重送都保存原因、操作者與 AuditLog。

## 10. 檢舉狀態 `ReportCaseStatus`

```text
Open → Assigned → InReview → Actioned | Rejected → Closed
```

- 補件使用 `RequestMoreInfo` Action、StatusHistory 與通知，案件維持 `InReview`。
- 成立以 `Actioned + ResolutionCode` 表達；不得新增 `Submitted/NeedMoreInfo/Founded/Unfounded`。
- 一般案件進專用檢舉佇列供 `CustomerService` 自領；個資、安全、詐欺、法律與其他高風險案件須由 `CustomerServiceSupervisor` 指派或覆核。

## 11. 退貨寄回物流邊界

- `ReturnShipment`／`ReturnShipmentEvent` 是退貨領域獨立 Aggregate，不共用 outbound `Shipment`。
- 第一版每個 ReturnRequest 同時最多一個有效寄回批次；事件以 `Source + ExternalEventId` 去重並 append-only。
- 物流事件驅動既有 ReturnStatus 的 `AwaitingShipment → InTransit → Received`，不得建立第二套 ReturnRequest 狀態。
- 本批不自行新增 ReturnShipment 狀態列舉；Kafen Schema 實作必須在 Migration 前依上述 ReturnStatus 邊界補齊並交由跨模組 Review。
