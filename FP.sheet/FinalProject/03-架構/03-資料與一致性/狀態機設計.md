...
- 完成 Endpoint 只在 Demo Profile 且 `Demo:SimulationEndpointsEnabled=true` 開放，需 Owner Member／有效 Guest Scope與 Antiforgery；`simulationKey` 同時作為此命令的唯一重播鍵與 Provider Event 鍵，不另要求 `Idempotency-Key` Header。Demo Profile 外不得提供相同寫入入口。
