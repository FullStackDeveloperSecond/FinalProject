...
本頁沿用 [[03-架構/資料字典-商品庫存與組裝]] 的 Profile。Identity 主鍵使用 `nvarchar(450)`；領域實體仍使用 `bigint identity`＋UUID v7 PublicId。所有 FK 預設 Restrict。
...
| `SupportAttachments`／MutableEntity | `SupportTicketId bigint`、`SupportMessageId bigint NULL`＋私有附件中繼資料 | 每案最多 3 個、每個 10 MB；StorageKey UX |
| `SupportAssignmentHistories`／Entity | `SupportTicketId bigint`、`FromAdminUserId/ToAdminUserId nvarchar(450) NULL`、`Action varchar(24)`、`Reason nvarchar(500) NULL`、`ActorAdminUserId nvarchar(450)`、`OccurredAtUtc datetime2(3)` | append-only；`IX_AssignmentHistories_TicketId_OccurredAtUtc` |
...
- 重複檢舉由同 Reporter＋Target＋Open Status 查詢阻擋；關閉後可建立新案件。

`ReportAttachments` 使用 MutableEntity，包含 `ReportCaseId` 及私有附件中繼資料；每案 3 個、每個 10 MB。檢舉與客服不共用可寫案件 Entity。
