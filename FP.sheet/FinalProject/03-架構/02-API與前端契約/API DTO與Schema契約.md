...
| 商品圖片上傳（multipart） | `file`（JPG／PNG／WebP，≤10 MB）＋ `altText?`(≤160)、`sourceUrl?`(≤1000)、`licenseName?`(≤100)、`licenseUrl?`(≤1000)；未給 altText 取檔名；成功 201 `AdminProductImageDto`（Ready） |
| `UpdateProductImageRequest` | `altText`(1..160)、`sortOrder`(0..9999)、`sourceUrl?`、`licenseName?`、`licenseUrl?`、`rowVersion` |
| `ProductImageActionRequest` | `rowVersion`；`actions/publish` 與 `DELETE` 共用。發布須 Alt／來源／授權齊備，否則 422 `image_metadata_incomplete`；已發布再發布為 no-op |
