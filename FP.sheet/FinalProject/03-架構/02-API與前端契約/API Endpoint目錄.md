...
| UC-COMPAT-01 SKU 相容性屬性 | `GET /api/v1/admin/compatibility-rules/skus/{skuPublicId}/attributes`；`PUT /api/v1/admin/compatibility-rules/skus/{skuPublicId}/attributes` | `CompatibilityRule.ManageWarnings`（CatalogManager／SuperAdmin） | 全量替換語意，DTO 依 [[03-架構/07-領域設計/相容性規則後台設計]] | `validation_failed`、`resource_not_found`、`concurrency_conflict` |
