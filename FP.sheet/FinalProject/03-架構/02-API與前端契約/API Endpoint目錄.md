...
| 管理員帳號與角色 | `GET/POST /api/v1/admin/administrators`；`GET /api/v1/admin/administrators/{id}`；`PUT /api/v1/admin/administrators/{id}/roles` | `RoleAssignment.Manage`；目前只授予完成 TOTP 的 SuperAdmin | 列表支援搜尋／狀態／角色／頁碼；建立直接提交 Email、初始密碼、顯示名稱、員工編號與角色；角色異動帶 RowVersion、原因及必要的 SuperAdmin 確認 | `admin_email_duplicate`、`admin_employee_code_duplicate`、`admin_role_invalid`、`last_super_admin_required`、`super_admin_confirmation_required`、`concurrency_conflict` |
...
| UC-ADM-INV-01 對帳 | `GET /api/v1/admin/inventory/reconciliation-cases`；`POST /api/v1/admin/inventory/reconciliation-cases/{id}/actions/acknowledge`；`POST /api/v1/admin/inventory/reconciliation-cases/{id}/actions/dismiss`；`POST /api/v1/admin/inventory/reconciliation-cases/{id}/actions/resolve` | InventoryManager／SuperAdmin | `InventoryReconciliationCaseQuery` → `PageResult<InventoryReconciliationCaseDto>`；acknowledge 只帶 RowVersion；dismiss／resolve 共用 `ReconciliationCaseResolutionRequest`＋RowVersion，成功 204；dismiss 不動庫存，resolve 以帳本重算值修正 Balance 並建立零差額 Adjustment，兩者都寫中央 Audit（`inventory_reconciliation.dismiss`／`resolve`） | `validation_failed`、`resource_not_found`、`inventory_reconciliation_case_not_open`、`concurrency_conflict`、`inventory_reconciliation_ledger_inconsistent` |
...
| UC-SUPPORT-01 後台明細 | `GET /api/v1/admin/support-tickets/{id}`；`POST /api/v1/admin/support-tickets/{id}/messages`；`POST /api/v1/admin/support-tickets/{id}/internal-notes`；`POST /api/v1/admin/support-tickets/{id}/actions/{action}` | 讀取依客服角色；公開 messages、internal-notes、`claim`、一般 `change-priority`、`change-status`、`cancel`、`reopen`：`SupportTicket.Handle`；`assign`、`transfer`、優先級覆核／覆寫：`SupportTicket.Supervise` | 公開回覆只允許已指派且未進入 Resolved／Closed／Cancelled 的案件，帶 RowVersion；一般客服限本人承辦，Supervisor 可處理任一已指派案件。首次人工公開回覆只記一次，與公開訊息、必要的 Assigned→InProgress 歷程、中央 Audit、Email／站內通知 Outbox 同次提交；Action 白名單維持 `claim`、`assign`、`transfer`、`change-priority`、`change-status`、`cancel`、`reopen` | `support_ticket_assignment_conflict`、`support_ticket_state_conflict`、`support_ticket_cancel_not_allowed`、`concurrency_conflict`、`resource_not_found` |
