...
本文件是 `customer-web` 與 `admin-web` 的第一版功能頁面契約，固定頁面 Route、角色入口、主要 Server State、API 對照及共同失敗狀態。商業規則仍以領域需求及使用案例為準；API 欄位、Policy 與錯誤碼分別以 [[03-架構/02-API與前端契約/API Endpoint目錄]]、[[03-架構/02-API與前端契約/API DTO與Schema契約]]、[[01-需求/角色與權限]] 及 [[03-架構/02-API與前端契約/API錯誤碼目錄]] 為準。
...
- API-M-01～12 的契約測試及 [[03-架構/08-測試與驗收/M功能測試案例目錄]] 既有案例能覆蓋成功、空白、錯誤、授權、併發與降級。
