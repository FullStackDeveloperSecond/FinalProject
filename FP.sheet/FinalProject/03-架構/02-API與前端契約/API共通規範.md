...
- 新增私人資源 Endpoint 必須遵守 [[03-架構/04-安全與檔案/安全與供應鏈強制驗收標準]]，在同一 PR 提供 User A／User B 負面整合測試與拒絕後無副作用證據。
...
穩定錯誤碼與 HTTP Status 對照見 [[03-架構/02-API與前端契約/API錯誤碼目錄]]；各端點另於 [[03-架構/02-API與前端契約/API Endpoint目錄]] 指定主要錯誤碼。
...
- 完整端點、Method、DTO 與 Schema 已收斂於 [[03-架構/02-API與前端契約/API Endpoint目錄]] 與 [[03-架構/02-API與前端契約/API DTO與Schema契約]]。
...
- Idempotency Record 每日清除已到期且非 `Processing`／調查保留的紀錄，排程見 [[03-架構/05-背景工作與維運/背景工作與Hangfire設計]]。
