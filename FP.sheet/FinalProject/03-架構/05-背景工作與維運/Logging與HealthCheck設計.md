...
- `Observability:FileLoggingEnabled=false` 只供測試或明確停用檔案 Sink；Console JSON 仍保留。Development 最低層級為 Debug，其他環境為 Information，ASP.NET Core 與 EF Core 框架雜訊最低為 Warning。
...
| `/health/ready` | 驗證 `Storage:DataRoot` 可寫，並由 EF Core 對 `DoSelectDb` 執行最小 `SELECT 1` 查詢 | 公開端點只回整體 `status`，不列路徑、連線字串、例外或依賴細節 |
...
- Hangfire 與 Brevo SMTP Adapter 已接入 Infrastructure；Hangfire 屬本機必要依賴並由 Readiness 驗證，Brevo 屬可降級外部依賴。OpenAI Live Adapter 尚未接入，`health-check.ps1` 不為未完成能力建立假結果。
