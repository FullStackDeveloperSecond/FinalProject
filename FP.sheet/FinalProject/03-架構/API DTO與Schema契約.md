...
本頁補足 [[03-架構/API Endpoint目錄]] 中具名 DTO 的欄位與上限。實際 OpenAPI 必須由 ASP.NET Core 產生並經 CI 比對；不得讓前端手寫另一套不同型別。
...
相容性 Request／Response 使用 [[03-架構/相容性規則後台設計]] 的 Item、Overall 與 Result 結構；公開版不接受 Draft Settings，且不回管理設定值。AI 商品搜尋 Schema、`existingParts` Union、上限與 Result Type 使用 [[03-架構/AI應用詳細設計]]。
...
`POST /orders` 的 `Idempotency-Key` Header 必填；Body 不重複保存 Key。宅配需 Address 且不得 COD；超取需 Store PublicId。
...
| `ReportResultDto` | `reportKey`、`title`、`timeBasis`、`timeZone`、`from/to`、`generatedAtUtc`、`asOfUtc`、`summary:{metricKey,value,unit}[]`、`series:{bucket,metrics}[]`、`rows:CursorPage<ReportRowDto>` |
