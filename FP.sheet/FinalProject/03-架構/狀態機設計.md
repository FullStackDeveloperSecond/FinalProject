...
本文件把訂單、付款、退款、組裝、物流、退貨、庫存保留與客服案件拆成獨立狀態機。內容以既有商業規則為基礎；標示「待確認」的部分是設計提案，確認前不得直接視為最終需求。
...
| 無 | 建立貨到付款訂單 | `Confirmed` | 只允許符合 COD 規則的配送方式；原子保留庫存 |
...
- 付款失敗或使用者取消某一次付款，不立即把訂單改成 `Cancelled`；在付款期限內可建立新的付款嘗試。此行為待確認。
- 顧客提出取消但需要人工審核時，建議另建 `OrderCancellationRequest`，不要把 `CancellationRequested` 塞入 `OrderStatus`。此行為待確認。
- 已出貨後不再使用取消，改走拒收、配送退回或退貨流程。
- `Completed` 後發生部分退款，訂單仍保持完成；退款結果由獨立狀態表達。

## 2. 付款狀態 `PaymentStatus`

原文件將 `RefundPending`、`PartiallyRefunded`、`Refunded` 放在付款狀態中。建議移除，因為它們描述的是退款，不是付款嘗試。
...
- 貨到付款訂單建立時沒有 `Paid` 付款紀錄；建議先建立 `AwaitingPayment` 的 COD 付款紀錄，在 `PickedUp` 時轉成 `Paid`。此行為待確認。

## 3. 退款狀態

建議把「訂單累計退款情況」與「單次退款交易結果」分開，避免一筆訂單有多次部分退款時無法正確表達。
...
既有「Draft → Submitted → Paid → AssemblyPending → ... → Shipped」混合了組裝清單、訂單、付款與物流。建議組裝狀態只描述組裝作業：
...
- 是否允許同一訂單多台組裝電腦各自擁有獨立 `AssemblyJob`，與 DOM-03 一併確認。
...
| `InTransit → Delivered` | 保存宅配送達時間，計算一般退貨期限 |
| `PickupReady → PickedUp` | 保存取貨時間；若為 COD，同步完成付款 |
| `DeliveryFailed → Returned` | 建立退回歷程；訂單、付款、庫存後續處理不得只靠物流狀態自動猜測 |

- 組裝電腦只有 `AssemblyStatus = ReadyToShip` 才能進入 `Shipped`。
- 第一版一張訂單是否可能拆成多張物流單尚未明確；建議限制為一張訂單一種配送方式、一張主要物流單，以控制 40 天範圍。此項待確認。
...
- `ReturnToStock`、`Damage` 或其他庫存處置細節仍由 DOM-08 補完。
...
狀態集合已確認如下；完整轉移、角色權限、SLA 計時與自動結案參數仍待完成。
...
- 尚待確認 `Closed` 後是否允許管理員重開、`Resolved` 自動轉 `Closed` 的天數、各轉移可執行角色、取消條件及 SLA 計時副作用。
...
以下決策確認後，才能將 DES-01 至 DES-06 標為完成：

1. 單次付款失敗/取消後，訂單是否保留到原期限並允許建立新的付款嘗試。
2. 需要人工審核的取消是否採獨立 `OrderCancellationRequest`，而不增加 `CancellationRequested` 訂單狀態。
3. COD 是否在訂單建立時建立 `AwaitingPayment` 付款紀錄，並於 `PickedUp` 時轉 `Paid`。
4. 是否採用 `OrderRefundStatus` 與 `RefundTransactionStatus` 兩層退款狀態。
5. 第一版是否限制一張訂單只有一種配送方式與一張主要物流單。
6. 多台組裝電腦是否建立多筆獨立 `AssemblyJob`，讓每台可以各自追蹤進度。
7. 退貨核准後，顧客應在幾天內寄回，逾時是否自動轉 `Cancelled`。
8. `Resolved` 幾天後自動轉 `Closed`，以及 `Closed` 是否允許管理員重開。
9. 客服案件各轉移的角色、取消條件及對 SLA 計時的副作用。
