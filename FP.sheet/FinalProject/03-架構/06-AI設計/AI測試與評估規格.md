...
截至 2026-08-30，`DoSelect.Application.Tests` 已把上述安全契約落實為零外部呼叫的 xUnit 測試，並建立由 AI Adapter 重用的 Application 與 SQL Server 安全邊界：

| 邊界 | 已驗證內容 | 尚未宣稱的內容 |
|---|---|---|
| 客服前置閘門 | 匿名、錯誤帳號類型、真正 GuestOrderAccess Cookie、功能關閉、同意拒絕／撤回、每日額度與併發最後一額、敏感內容及 Owner 拒絕；拒絕路徑模型零呼叫，安全路徑只預留與呼叫一次；M-19 同意與降級瀏覽器旅程已合併 | 尚未完成 live 品質、延遲與成本證據 |
...
| 故障降級 | 客服 Adapter 已驗證 429／5xx／網路／格式錯誤最多重試一次、其他 4xx 與模型轉人工不重試；商品搜尋 `product-search-v9` 沿用單次意圖呼叫、5 秒逾時、零同步重試、敏感輸入零模型呼叫、互動保存失敗 Fail Closed 及 `keywordSearch` 明確降級，推薦理由由後端核准事實確定性產生且零模型呼叫；payload 固定 `reasoning.effort: none`、`text.verbosity: low` 且不送 `service_tier`。Semantic Key 採正式大寫＋分類精確 allowlist；`STORAGE_CAPACITY_GB` 與 RAM Key 分離，TB→GB 由後端正規化；補問或既有零件確認時 Runner 不得虛構推薦。明確商品標籤可映射用途，指定單一零件／周邊且已給分類與預算時，即使提及現有硬體也維持 `SingleProduct`；模型提出結構化既有零件後，由應用層另行確認。僅 `zh-TW` 的明確口語金額由 Adapter 保存為最高預算，衝突時保留安全上限並補問，模糊金額不強制覆寫。顧客可見理由承接用途、預算、硬性規格與偏好，並拒絕內部 Enum／代碼／Fixture ID／後端術語；InvalidOutput JSONL 只含固定 reason code／field，不含 raw output | PR #120 已合併；DEC-BATCH-062 的 v8 六案 Smoke 自動 Gate 與人工 6／6 均通過。v8 三輪 Release baseline 整體 `FAIL` 後，DEC-BATCH-064 已完成五案 v9 零成本修正；`product-search-v9 + support-v3` 的新 Live Smoke 與人工覆核仍待獨立授權 |
...
- 120 筆繁中實際案例、Fixture、Schema、Grader 與本機／CI deterministic 驗證已建立。現行資料集為 `zh-TW-v1.0.6-draft`／Fixture `v1.0.4`；v1.0.2 政策案例及 v1.0.3 創作者案例覆核仍有效。grader `deterministic-v1.1.5` 精確核對分類、結構化規格、偏好概念與既有零件提案／確認階段。依 DEC-BATCH-064，014／015／020／021／023 已按 v8 三輪原始證據修正；development 009 與 challenge 030 增加非 Release 泛化保護，challenge 只作獨立觀測，不用於調整 Prompt。120 筆案例定義均為 `approved`；任何新的 v9／support-v3 顧客可見輸出仍須逐案人工覆核。
...
- DEC-BATCH-064 已依該 baseline 的原始逐輪證據完成商品五案零成本修正：Prompt／Dataset／Grader 升為 `product-search-v9`／`zh-TW-v1.0.6-draft`／`deterministic-v1.1.5`，Fixture 與 120 案總數不變。聚焦 7／7、Application AI 150／150、受影響 Infrastructure 45／45、資料同步／Schema／隱私掃描及三輪 66 次 Dry Run 通過，外部模型呼叫與成本為 0。正式報告：`FP.dev/evals/ai/v1/results/2026-09-06-v9-five-case-zero-cost-remediation.md`。下一 Gate 是另行授權的小型 Live Smoke 與顧客視角人工覆核；通過後才決定是否重跑完整 baseline。
