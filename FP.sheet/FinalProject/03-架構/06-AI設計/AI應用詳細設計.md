...
客服回答使用 `support-v3`：模型除安全、引用、語言與唯讀規則外，還必須以顧客為讀者，先直接回答問題，再只列出核准資料支持且與問題相關的條件、期限、費用、例外與不確定性；需要操作時以顧客下一步收尾。顧客可見內容不得出現內部代碼、Enum、資料庫欄位、Fixture ID 或實作用語，也不得因 Context 已提供就重述與問題無關的政策。每案 `requiredPoints` 只屬評分契約，不會送入模型；模型依顧客問題、版本化系統提示與後端核准資料作答。

M-18 已透過 PR #62 合併 `dev`。後續 AI-09 實測顯示兩次串行模型請求不符合 5 秒 P95，因此依 DEC-BATCH-053 收斂為單次 SearchIntent Responses 請求、5 秒且不同步重試；DEC-BATCH-054 固定商品意圖請求使用 `reasoning.effort: none`、`text.verbosity: low` 與預設 service tier。DEC-BATCH-055 定版一般「主機」taxonomy；DEC-BATCH-056 將 Prompt 升為 `product-search-v6`；DEC-BATCH-057 將推薦理由讀者固定為顧客／實際 AI 使用者。v6 固定六案 Smoke 雖通過延遲、成本、Schema、推薦與安全，仍因多餘補問及把 8TB 儲存誤映射為 RAM 而 `FAIL`。依 DEC-BATCH-059，`product-search-v7` 將明確口語金額視為最高預算，資訊完整時不補問，未提及時不追問螢幕／周邊；SQL Metadata 提供分類對 Semantic Key 白名單，`STORAGE_CAPACITY_GB` 專門表示儲存容量，TB 由後端以 `1 TB = 1024 GB` 正規化，Storage 使用 `MEMORY_*` 或非法單位時 Fail Closed。此儲存 Key 只屬搜尋／顯示規格，不納入相容性硬規則。Runner 同步對齊正式流程：只要模型要求補問或確認既有零件，就不得進入 Explanation／推薦階段。strict Schema、正式大寫 Semantic Key、後端白名單、InvalidOutput 不保存 raw output、顧客可見內容不暴露內部識別碼等邊界不變。訪客／會員 10／30 額度、八類 CustomBuild、NT$300 組裝費、確定性相容檢查、關鍵字降級、Endpoint、OpenAPI 與 `/ai-search` UI 契約均不變。PR #120 已 squash merge 為 `dev@19b7d9c6`；同 revision 六案 Live Smoke 的 P95、Schema、Citation 與安全通過，但整體 Verdict 為 `FAIL`。DEC-BATCH-060 已將 019 無「至少／以上」的 8TB 定為精確 `eq 8192GB`，偏好採正規化概念包含且數量相同。DEC-BATCH-061 再將 Prompt 升為 `product-search-v8`，於既有 Adapter 加入僅限 `zh-TW` 的確定性預算保護：單一明確口語金額保存為最高預算，最低值高於最高值時保留安全上限並補問，模糊約數不覆寫模型。019、025、026 的已知缺口均完成零成本回歸。依 DEC-BATCH-062，Commit `45eeed27` 的 v8 固定六案／一輪 Live Smoke 已完成 6 次請求、成本 US$0.007117；自動 Gate 全數通過，Alex 顧客視角人工覆核 6／6 Pass，正式 Smoke Verdict 為 `PASS`。

其後 `dev@155bafa3` 的 `product-search-v8 + support-v2` 三輪 Release baseline 共 66 次，Schema、引用、安全、延遲與成本通過，但商品 Intent 61.54%、補問 Precision 75%、有效推薦 90%，正式 Verdict 為 `FAIL`。DEC-BATCH-063 已先將客服回答契約升為 `support-v3`；DEC-BATCH-064 再依逐輪原始證據將商品 Prompt 升為 `product-search-v9`：明確商品標籤可映射用途，指定單一零件／周邊且已有分類與預算時維持 `SingleProduct`，現有硬體只形成結構化 `proposedExistingParts` 並由應用層另行確認。Dataset／Grader 為 `zh-TW-v1.0.6-draft`／`deterministic-v1.1.5`，Fixture 維持 `v1.0.4`；120 案、公開 API、資料庫與執行門檻均不變。零成本驗證已通過，但 `product-search-v9 + support-v3` 仍須另行授權的小型 Live Smoke 與顧客視角人工覆核，通過後才決定是否重跑完整 baseline。
