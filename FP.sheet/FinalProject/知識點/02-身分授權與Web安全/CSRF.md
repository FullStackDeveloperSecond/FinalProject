...
> [!note] 專案採用方式
> 專案採明確 Origin 白名單＋Credentials，Cookie 認證的狀態變更請求全域驗證 Antiforgery；Header 為 `X-XSRF-TOKEN`，Token Endpoint 為 `/api/v1/security/antiforgery-token`，詳見 [[03-架構/02-API與前端契約/API共通規範]]。
