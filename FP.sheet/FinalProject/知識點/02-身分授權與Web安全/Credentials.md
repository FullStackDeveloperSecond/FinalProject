...
Credentials Cookie 會由瀏覽器自動附加，正是 CSRF 風險的來源之一。因此 Cookie SPA 不能只設定 `include` 與 CORS，還要在改變狀態的請求帶 [[知識點/02-身分授權與Web安全/Antiforgery|Antiforgery Token]]：
...
> [!warning] 專案決策邊界
> DEC-P50 已確認明確 Origin 白名單＋Credentials＋Antiforgery；實際 Origin、Cookie 屬性與前端 wrapper 契約仍待環境及實作設計，正式邊界見 [[03-架構/02-API與前端契約/API共通規範]]。
