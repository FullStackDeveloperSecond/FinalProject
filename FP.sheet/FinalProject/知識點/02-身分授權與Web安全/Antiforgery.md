...
[[知識點/02-身分授權與Web安全/CSRF|CSRF]] 是攻擊類型；Antiforgery 是 ASP.NET Core 用來防禦這類攻擊的具體機制。
...
> [!note] 專案採用方式
> 全域保護 Cookie 認證的非安全方法；前端由 `GET /api/v1/security/antiforgery-token` 取得 Request Token，以 `X-XSRF-TOKEN` Header 傳送，失敗回 400＋`antiforgery_validation_failed`。精確 ASP.NET Core 註冊程式屬實作，正式邊界見 [[03-架構/02-API與前端契約/API共通規範]]。
