...
| `Frontend` Matrix | 兩個 Vue 專案分別執行 `npm ci`、Typecheck、零警告 Lint、Vitest、Build 與 Production Dependency Audit |
| `CI Required` | 彙總所有必要 Job；供 `main`／`dev` Branch Protection 使用 |
...
目前尚未完成正式 CD。本專案第一版只需在單一 Windows 展示電腦執行，不部署公網；Coverage 失敗門檻、OpenAPI Client Diff、SQL Server Provider-backed 整合測試、自動 Secret Scanner、部署 Artifact、Environment、Migration Deployment 及自動回復仍須依追蹤項目逐步落實。

> [!note] 專案決策邊界
> 已完成的是 GitHub Actions 基礎 CI 與 Branch Protection，不是正式環境自動部署。實際通過項目及尚待工作以 [[03-架構/測試策略]]、[[03-架構/本機開發環境與版本基線]] 和 [[05-規劃/未完成項目追蹤表]] 為準。
