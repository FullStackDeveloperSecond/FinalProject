...
> [!note] 專案決策邊界
> 專案使用 `/health/live` 與 `/health/ready`；v1 目標以 SQL Server 與必要本機目錄作為 Ready 條件，OpenAI／Brevo可 Degraded。SH-11A 第一階段目前只實作本機資料根目錄可寫探針，SQL／Migration／Hangfire 待 Infrastructure 完成後加入。正式內容見 [[03-架構/05-背景工作與維運/Logging與HealthCheck設計]]。
