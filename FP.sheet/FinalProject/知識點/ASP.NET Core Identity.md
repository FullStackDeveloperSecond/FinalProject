...
Cookie 會自動隨請求送出，因此所有會改變狀態的瀏覽器 API 必須搭配 [[知識點/CSRF|Antiforgery／CSRF 防護]]。Vue 與 API 不同 Origin 時，另需設定 [[知識點/CORS|CORS]]。
...
> [!warning] 專案決策邊界
> DEC-P47～DEC-P49 已確認採 Identity＋HttpOnly Cookie、會員與管理員獨立 Cookie Scheme，以及不同 Session 期限；正式規則見 [[03-架構/API共通規範]]。
