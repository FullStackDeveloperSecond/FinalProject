...
public sealed class ReturnRequestConfiguration : IEntityTypeConfiguration<ReturnRequest>
{
    ...
    {
        b.ConfigureMutablePublicEntity("ReturnRequests"); b.Property(x => x.ReturnNumber).HasMaxLength(32).IsRequired(); b.HasIndex(x => x.ReturnNumber).IsUnique().HasDatabaseName("UX_ReturnRequests_ReturnNumber"); b.Property(x => x.RequesterUserId).HasMaxLength(450); b.Property(x => x.Status).HasConversion<string>().HasMaxLength(24).IsUnicode(false).IsRequired(); b.Property(x => x.Priority).HasConversion<string>().HasMaxLength(16).IsUnicode(false).HasDefaultValue(CasePriority.Normal).IsRequired(); b.Property(x => x.ReasonCode).HasMaxLength(64).IsUnicode(false).IsRequired(); b.Property(x => x.Description).HasMaxLength(1000).IsRequired(); b.Property(x => x.AssigneeAdminUserId).HasMaxLength(450); b.Property(x => x.ReviewedByAdminUserId).HasMaxLength(450); b.Property(x => x.RequestedAtUtc).HasPrecision(3); b.Property(x => x.ApprovedAtUtc).HasPrecision(3); b.Property(x => x.ReceivedAtUtc).HasPrecision(3); b.Property(x => x.ClosedAtUtc).HasPrecision(3); b.Property(x => x.ReturnShipmentDueAtUtc).HasPrecision(3); b.Property(x => x.AssemblyFeeDisposition).HasConversion<string>().HasMaxLength(32).IsUnicode(false); b.Property(x => x.ReturnShippingCost).HasPrecision(18, 2);
        b.HasIndex(x => new { x.OrderId, x.Status }).HasDatabaseName("IX_ReturnRequests_OrderId_Status"); b.HasIndex(x => new { x.Status, x.Priority, x.AssigneeAdminUserId, x.UpdatedAtUtc }).HasDatabaseName("IX_ReturnRequests_Status_Priority_AssigneeAdminUserId_UpdatedAtUtc"); b.HasOne<Order>().WithMany().HasForeignKey(x => x.OrderId).OnDelete(DeleteBehavior.Restrict); User(b, nameof(ReturnRequest.RequesterUserId)); User(b, nameof(ReturnRequest.AssigneeAdminUserId)); User(b, nameof(ReturnRequest.ReviewedByAdminUserId)); b.ToTable("ReturnRequests", t => { t.HasCheckConstraint("CK_ReturnRequests_PolicyVersion", "[PolicyVersion] > 0"); t.HasCheckConstraint("CK_ReturnRequests_RefundTrustedInputs", "([AssemblyFeeDisposition] IS NULL AND [ReturnShippingCost] IS NULL) OR ([AssemblyFeeDisposition] IS NOT NULL AND [ReturnShippingCost] IS NOT NULL)"); t.HasCheckConstraint("CK_ReturnRequests_ReturnShippingCost", "[ReturnShippingCost] IS NULL OR [ReturnShippingCost] >= 0"); });
    }
