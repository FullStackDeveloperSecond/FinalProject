...
public sealed class ReturnAttachmentConfiguration : IEntityTypeConfiguration<ReturnAttachment>
{
    ...
    {
        b.ConfigureMutablePublicEntity("ReturnAttachments"); b.Property(x => x.UploadedByUserId).HasMaxLength(450).IsRequired(); b.Property(x => x.OriginalFileName).HasMaxLength(255).IsRequired(); b.Property(x => x.StorageKey).HasMaxLength(500).IsRequired(); b.HasIndex(x => x.StorageKey).IsUnique().HasDatabaseName("UX_ReturnAttachments_StorageKey"); b.Property(x => x.Extension).HasMaxLength(10).IsUnicode(false).IsRequired(); b.Property(x => x.MimeType).HasMaxLength(100).IsUnicode(false).IsRequired(); b.Property(x => x.Sha256).HasColumnType("binary(32)").IsRequired(); b.Property(x => x.ScanStatus).HasConversion<string>().HasMaxLength(20).IsUnicode(false).IsRequired(); b.Property(x => x.ScannedAtUtc).HasPrecision(3); b.Property(x => x.RetentionUntilUtc).HasPrecision(3); b.Property(x => x.LegalHold).HasDefaultValue(false).IsRequired(); b.Property(x => x.DeletedAtUtc).HasPrecision(3); b.HasIndex(x => x.ReturnRequestId).HasDatabaseName("IX_ReturnAttachments_ReturnRequestId"); b.HasOne<ReturnRequest>().WithMany().HasForeignKey(x => x.ReturnRequestId).OnDelete(DeleteBehavior.Restrict); b.HasOne<ApplicationUser>().WithMany().HasForeignKey(x => x.UploadedByUserId).OnDelete(DeleteBehavior.Restrict); b.ToTable("ReturnAttachments", t => t.HasCheckConstraint("CK_ReturnAttachments_FileSize", "[FileSizeBytes] >= 1 AND [FileSizeBytes] <= 10485760"));
    }
}
