...
目前 Repository 雖與 `origin/dev` 同一 commit，但工作目錄仍有尚未提交的程式與文件變更。因此目前**不可建立最終壓縮檔**：

- 直接對 commit 做 `git archive` 會漏掉未提交修改。
...
1. **原始碼版本**：目前有大量未提交變更；必須先決定交付的是哪個 commit，不能以「現在資料夾看起來能跑」代替版本固定。
