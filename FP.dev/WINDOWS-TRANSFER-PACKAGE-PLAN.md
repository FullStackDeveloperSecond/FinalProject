...
規劃日期：2026-09-13；核准日期：2026-09-14（Asia/Taipei）
交付基準：`origin/dev`；實際完整 commit SHA、建立時間、檔案大小與驗證結果以壓縮包根目錄的 `PACKAGE-MANIFEST.txt` 為準。
...
不得直接壓縮日常工作目錄，以免混入 `.run`、套件快取、測試產物、本機設定或其他不應交付的內容。每次建立交付包都必須完成：
...
本次依建議採用下列預設值；後續若有不同需求，必須另案重新評估並建立新交付包：

1. **套件型態**：可連網安裝／還原，不夾帶會過期的第三方安裝媒體或套件快取。
2. **資料範圍**：以 Migration 與 Seed 建立全新 Development／Demo 資料庫，不搬移現有 MDF／LDF、備份或使用者資料。
3. **原始碼版本**：以封裝當下已推送的 `origin/dev` 完整 SHA 建包。
4. **目標電腦**：預設 Windows x64、可取得本機管理員權限、可安裝 `SQL2025` named instance，且沒有必須沿用的既有 SQL Instance／Port 限制。

本次採可連網安裝／還原、全新 Development／Demo 資料庫，以及已推送的 `origin/dev` commit。目標電腦預設為 Windows x64、可取得本機管理員權限並可安裝 `SQL2025` 具名執行個體。
...
封裝只建立新的交付目錄與 ZIP；不複製資料庫、不輸出 Secret，也不清理日常工作目錄。
