...
- AI 客服 Responses Adapter、M-19 與 M-18 搜尋垂直切片均已合併 `dev`；現行商品搜尋維持單次 SearchIntent Responses Adapter，推薦理由由後端核准候選事實確定性產生，其餘白名單候選、既有零件確認閘門、降級路徑與 `/ai-search` UI 契約不變。Live Runner 已建立 Dry Run、明確執行旗標、User Secrets、成本停止線與結果產物；最新完整 v10／support-v4 baseline 因 1 筆人工政策矛盾維持正式 `FAIL`；現行 support-v5／v1.0.10／v1.1.8 候選修正須完成 PR 驗證與合併，再進行 revision-pinned 聚焦 Live 復驗與完整 baseline。
