...
- AI 客服 Responses Adapter、M-19 與 M-18 搜尋垂直切片均已合併 `dev`；現行商品搜尋維持單次 SearchIntent Responses Adapter，推薦理由由後端核准候選事實確定性產生，其餘白名單候選、既有零件確認閘門、降級路徑與 `/ai-search` UI 契約不變。Live Runner 已建立 Dry Run、明確執行旗標、User Secrets、成本停止線與結果產物；v8 小型 Smoke 已通過，v8 三輪 66 次 Release baseline 已執行且正式 `FAIL`，現行 v9／support-v3 候選只完成零成本驗證，尚待另行授權的新小型 Smoke。
