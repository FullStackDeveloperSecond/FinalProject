...
目前商品意圖 Prompt 為 `product-search-v8`：沿用單次 Responses 意圖請求、5 秒零同步重試、`reasoning.effort: none`、`text.verbosity: low` 與預設 service tier；單一且明確的繁中口語金額可由後端確定性保存為最高預算，最低值高於最高值時保留安全上限並產生衝突補問，模糊約數或範圍不強制覆寫。Prompt 同時避免把儲存情境誤當一般用途、資訊完整時不追問未提及的螢幕／周邊，並以分類對應 Semantic Key 白名單驗證規格。`STORAGE_CAPACITY_GB` 專供儲存容量，TB 在後端以 `1 TB = 1024 GB` 正規化；RAM 仍使用 `MEMORY_*`。strict Schema、精確大寫 Semantic Key、白名單與後端驗證仍為內部正確性邊界；顧客可見的推薦理由只使用在地化名稱與自然語言，直接承接用途、預算、硬性規格、軟性偏好及核准 Badge，不顯示 SearchIntent enum、Semantic Key、Fixture ID、品牌／分類代碼或「後端候選」術語。InvalidOutput 只保存固定原因碼與欄位名稱，不保存 raw output。客服 Prompt 為 `support-v3`：安全與引用邊界維持不變，新增顧客回答規格，要求先直接回答，再只補充核准資料支持且與問題相關的條件、期限、費用、例外與不確定性，必要時以顧客下一步收尾，並禁止內部代碼、Enum、資料庫欄位、Fixture ID 與實作用語。人工覆核表會標出兩個 Prompt 版本、客服每案實際模型可用的核准來源及核准資料，並明示必要回答重點只供評分、不送入模型；商品案例另外集中列出意圖模型實際取得的分類／品牌／Semantic Key Metadata，逐案列出確實進入推薦階段的後端核准商品事實，且清楚區分後者不屬模型輸入。grader `deterministic-v1.1.4` 精確核對分類與結構化必要規格，偏好則在 Unicode 英數正規化後核對概念包含且數量相同。資料集現行版本為 `zh-TW-v1.0.5-draft`，Fixture 為 `v1.0.4`；120 案案例定義已核准，任何新顧客可見輸出仍須重新人工覆核。
...
- AI 客服 Responses Adapter、M-19 與 M-18 搜尋垂直切片均已合併 `dev`；M-18 的現行工作樹將搜尋收束為單次 SearchIntent Responses Adapter，推薦理由改由後端核准候選事實確定性產生，其餘白名單候選、既有零件確認閘門、降級路徑與 `/ai-search` UI 契約不變。Live Runner 已建立 Dry Run、明確執行旗標、User Secrets、成本停止線與結果產物；v8 小型 Smoke 已通過，三輪 66 次 Release baseline 尚未授權。
