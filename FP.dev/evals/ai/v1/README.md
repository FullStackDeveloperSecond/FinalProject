...
- 同日依 DEC-BATCH-061 完成 `product-search-v8` 中文預算確定性保護：025 即使模型漏掉「三萬五」仍保存最高預算並移除純預算補問；026 即使模型回傳 null 仍保留 NT$15,000 安全上限並提出衝突補問；模糊金額不強制判定。Adapter 20／20、Live Evaluation 23／23、Build、Format、120 筆驗證及 66 次 Release dry run 通過，未呼叫 OpenAI。完整零成本證據見 [`results/2026-09-06-v8-budget-guard-remediation.md`](results/2026-09-06-v8-budget-guard-remediation.md)；新 v8 Live Smoke 仍須另行授權。
