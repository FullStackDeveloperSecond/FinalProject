...
- AI 客服 Responses Adapter、M-19 與 M-18 搜尋垂直切片均已合併 `dev`；現行商品搜尋維持單次 SearchIntent Responses Adapter，推薦理由由後端核准候選事實確定性產生，其餘白名單候選、既有零件確認閘門、降級路徑與 `/ai-search` UI 契約不變。Live Runner 已建立 Dry Run、明確執行旗標、User Secrets、成本停止線與結果產物；v9／support-v3 三輪 66 次 Release baseline 與首次 v10／support-v4 五案 smoke 都保留為正式 `FAIL`；現行 v1.0.9／v1.1.7 候選修正需先完成 PR 驗證與合併，再進行 revision-pinned 聚焦 Live 復驗。
