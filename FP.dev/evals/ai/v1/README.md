...
目前商品意圖 Prompt 為 `product-search-v10`：沿用單次 Responses 意圖請求、5 秒零同步重試、`reasoning.effort: none`、`text.verbosity: low` 與預設 service tier；單一且明確的繁中口語金額可由後端確定性保存為最高預算，最低值高於最高值時保留安全上限並產生衝突補問，模糊約數或範圍不強制覆寫。Prompt 同時避免把儲存情境誤當一般用途、資訊完整時不追問未提及的螢幕／周邊，並以分類對應 Semantic Key 白名單驗證規格；明確的商品標籤可映射對應用途，「遊戲美術」等創作職務仍按創作用途處理。顧客明確指定單一零件／周邊且提供預算與分類時，即使提及現有硬體也維持 `SingleProduct`；模型只提出結構化既有零件，由應用層另行取得確認，不追問整台電腦用途，且頂層搜尋條件會移除與 proposal 完全重複的既有零件規格。`STORAGE_CAPACITY_GB` 專供儲存容量，TB 在後端以 `1 TB = 1024 GB` 正規化；未含上下界措辭的明確容量採 `eq`，明說 SSD 時保留為硬條件；RAM 仍使用 `MEMORY_*`。明確「配／組／組裝」詞會窄範圍覆寫誤判的 Prebuilt 分類。strict Schema、精確大寫 Semantic Key、白名單與後端驗證仍為內部正確性邊界；顧客可見的推薦理由只使用在地化名稱與自然語言，直接承接用途、預算、硬性規格、軟性偏好及核准 Badge，不顯示 SearchIntent enum、Semantic Key、Fixture ID、品牌／分類代碼或「後端候選」術語。單品屬於固定相容性元件類別時，回答會明示安裝前仍需核對現有設備規格，且商品卡沒有核准證據時不宣稱軟性偏好已被滿足。InvalidOutput 只保存固定原因碼與欄位名稱，不保存 raw output。客服 Prompt 為 `support-v4`：安全與引用邊界維持不變，要求先直接回答，再只補充核准資料支持且與問題相關的條件、期限、費用、例外、不確定性，以及適用的付款、履約管道與資格限制，必要時以顧客下一步收尾，並禁止內部代碼、Enum、資料庫欄位、Fixture ID 與實作用語。人工覆核表會標出兩個 Prompt 版本、客服每案實際模型可用的核准來源及核准資料，並明示必要回答重點只供評分、不送入模型；商品案例另外集中列出意圖模型實際取得的分類／品牌／Semantic Key Metadata，逐案列出確實進入推薦階段的後端核准商品事實，且清楚區分後者不屬模型輸入。grader `deterministic-v1.1.6` 除精確核對分類、結構化必要規格、既有零件提案與偏好概念，也以未送入模型的 `requiredFacts` 驗證指定客服政策必要事實；資料集現行版本為 `zh-TW-v1.0.8-draft`，Fixture 為 `v1.0.4`。120 案案例定義已核准，任何新顧客可見輸出仍須重新人工覆核。
...
- AI 客服 Responses Adapter、M-19 與 M-18 搜尋垂直切片均已合併 `dev`；現行商品搜尋維持單次 SearchIntent Responses Adapter，推薦理由由後端核准候選事實確定性產生，其餘白名單候選、既有零件確認閘門、降級路徑與 `/ai-search` UI 契約不變。Live Runner 已建立 Dry Run、明確執行旗標、User Secrets、成本停止線與結果產物；v9／support-v3 三輪 66 次 Release baseline 已執行且正式 `FAIL`，v10／support-v4 修正先進行零成本 PR 驗證，合併後的付費 Smoke／baseline 需另行授權。
