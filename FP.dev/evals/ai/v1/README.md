...
目前商品意圖 Prompt 為 `product-search-v12`：沿用單次 Responses 意圖請求、5 秒零同步重試、`reasoning.effort: none`、`text.verbosity: low`、strict Schema、catalog／Semantic Key 白名單、Fast 層級與後端 Fail Closed。明確預算、分類、容量、SSD 及質性偏好必須保留，但已由 purpose、budget、brand、required spec、category／keyword 或 proposed existing part 精確表達者不得重複；未確認既有零件不得推導目標商品 hard spec，相容性仍在確認後由應用規則計算。顧客可見理由只使用核准候選事實與在地化名稱，不顯示內部代碼或 raw invalid output。客服 Prompt 為 `support-v7`：沿用核准來源、唯讀工具、引用、退貨政策、跨帳號與顧客語言邊界；核准資料或安全拒絕加官方流程足以回答時 `needsHumanSupport` 必須為 false，不得只因寫入、決策或送件需由人工／正式流程執行就升級；真正無法安全回答或引導時仍維持 true 與 Adapter Fail Closed。grader `deterministic-v1.1.9` 及所有既有門檻不變。資料集現行版本為 `zh-TW-v1.0.12-draft`，Fixture 為 `v1.0.4`；020 明列不得有未確認衍生規格，022 明列安靜偏好。120 案仍須生成、Schema、隱私與人工 Gate，任何新顧客可見輸出均須重新人工覆核。
...
- AI 客服 Responses Adapter、M-19 與 M-18 搜尋垂直切片均已合併 `dev`；DEC-BATCH-067 的 product-search-v12／support-v7／Dataset v1.0.12 bounded remediation 已套用。120 案生成／Schema／隱私檢查、Application 616／616、精確 Infrastructure AI 100／100、Release build（0 warning／0 error）、format 與 66-request dry-run 均通過，未呼叫 Live API；6／6 source Security diff scan 為 complete，0 finding／0 deferred。仍須完成 commit、push、revision-pinned review、Required CI 與合併後四案聚焦 Live，只有 focused 12／12 通過才可重跑完整 baseline。
