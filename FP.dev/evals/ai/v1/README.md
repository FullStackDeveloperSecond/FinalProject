...
目前商品意圖版本為 `product-search-v13`：沿用 v12 單次 Responses 意圖請求、5 秒零同步重試、`reasoning.effort: none`、`text.verbosity: low`、strict Schema、catalog／Semantic Key 白名單、Fast 層級與後端 Fail Closed；並在既有 deterministic budget guard 中，僅於無貨幣「千」界線緊鄰原文商品 keyword 時恢復模型漏掉的明確預算。明確預算、分類、容量、SSD 及質性偏好必須保留，但已由 purpose、budget、brand、required spec、category／keyword 或 proposed existing part 精確表達者不得重複；未確認既有零件不得推導目標商品 hard spec，相容性仍在確認後由應用規則計算。顧客可見理由只使用核准候選事實與在地化名稱，不顯示內部代碼或 raw invalid output。客服 Prompt 為 `support-v7`：沿用核准來源、唯讀工具、引用、退貨政策、跨帳號與顧客語言邊界；核准資料或安全拒絕加官方流程足以回答時 `needsHumanSupport` 必須為 false，不得只因寫入、決策或送件需由人工／正式流程執行就升級；真正無法安全回答或引導時仍維持 true 與 Adapter Fail Closed。grader `deterministic-v1.1.9` 及所有既有門檻不變。資料集現行版本為 `zh-TW-v1.0.13-draft`，Fixture 為 `v1.0.4`；020 明列不得有未確認衍生規格，022 明列安靜偏好，SUPPORT-POLICY-013 的必要事實只要求顧客詢問的瑕疵七日例外。120 案仍須生成、Schema、隱私與人工 Gate，任何新顧客可見輸出均須重新人工覆核。
...
- AI 客服 Responses Adapter、M-19 與 M-18 搜尋垂直切片均已合併 `dev`；DEC-BATCH-069 的 product-search-v13 修正進行中。Dataset v1.0.13、support-v7、grader v1.1.9、模型、timeout、retry 與安全門檻不變；合併前須完成全部零成本驗證與 Security review，合併後先跑 SEARCH-NOVICE-023 三輪 focused，通過才可重跑完整 baseline。
