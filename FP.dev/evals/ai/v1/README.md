...
目前商品意圖 Prompt 為 `product-search-v5`：沿用 v4 的單次 Responses 意圖請求、5 秒零同步重試與後端確定性推薦理由，並依 DEC-BATCH-055 明確規定一般「主機」無用途／效能／組裝詞時採 PrebuiltComputer，用途＋預算或明示組裝才採 CustomBuild；「遊戲美術」等職稱不得自動推論 Gaming。依 DEC-BATCH-054，商品 SearchIntent 固定使用 `reasoning.effort: none`、`text.verbosity: low` 與預設 service tier；strict Schema、白名單與後端驗證仍為正確性邊界。InvalidOutput 只保存固定原因碼與欄位名稱，不保存 raw output。客服維持 `support-v2`。資料集現行版本為 `zh-TW-v1.0.3-draft`，Fixture 為 `v1.0.3`。
