...
目前商品意圖 Prompt 為 `product-search-v11`：沿用單次 Responses 意圖請求、5 秒零同步重試、`reasoning.effort: none`、`text.verbosity: low`、strict Schema、catalog／Semantic Key 白名單與後端 Fail Closed，只對商品搜尋使用可回復的 `service_tier: fast`。明確預算、分類、容量與 SSD 硬需求由模型與既有確定性保護共同保存；既有零件只進入待使用者確認的 proposal，頂層搜尋條件移除完全重複的規格或完整名稱偏好，但保留較長的待購商品能力描述。顧客可見理由只使用核准候選事實與在地化名稱，不顯示內部代碼或 raw invalid output。客服 Prompt 為 `support-v6`：沿用核准來源、唯讀工具、引用、退貨政策與顧客語言邊界，並明定跨帳號請求只能由該帳號持有人自行登入或聯絡客服，不得叫目前請求者登入其他會員帳號或使用其憑證。grader `deterministic-v1.1.9` 保留所有既有閾值，偏好概念只新增「正規化後位於開頭的需要」等值處理，並以未送入模型的 `requiredFacts` 阻擋觀察到的跨帳號登入措辭。資料集現行版本為 `zh-TW-v1.0.11-draft`，Fixture 為 `v1.0.4`；`SEARCH-NOVICE-021` 明確 SSD 期待已與產品規則一致。120 案定義仍須生成、Schema、隱私與人工 Gate，任何新顧客可見輸出均須重新人工覆核。
...
- AI 客服 Responses Adapter、M-19 與 M-18 搜尋垂直切片均已合併 `dev`；現行商品搜尋維持單次 SearchIntent Responses Adapter、Fast 層級、後端核准候選理由、白名單、既有零件確認與安全降級。DEC-BATCH-066 的 support-v6／Dataset／grader 修正已完成零成本驗證與 6／6 source security review，仍須走 commit、push、revision-pinned review、Required CI 與合併後聚焦 Live；只有聚焦通過才可重跑完整 baseline。
