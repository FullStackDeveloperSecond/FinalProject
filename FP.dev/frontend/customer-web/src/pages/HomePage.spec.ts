...
describe('HomePage', () => {
  ...
  it('offers the three beginner entry points', () => {
    ...
    const links = wrapper.findAll('.home-hero .home-step__link')
    expect(links.map(link => link.attributes('href'))).toEqual(['/ai-search', '/products', '/ai-search'])
    expect(wrapper.find('.home-hero__actions').exists()).toBe(false)
  })
