...
describe('8. 首頁保留九個語意圖示，並補上流程導覽感', () => {
  ...
  it('三個步驟都有圖示，型別與資料結構不倒退', () => {
    const steps = [...homeSource.matchAll(/\{ step: '[^']+', title: '[^']+', body: '[^']+', icon: '([a-z-]+)', to: '[^']+' \}/g)]
  ...
  it('Hero 素材是真的檔案，不是 base64 也不是 CDN', () => {
    const art = join(customerRoot, 'public', 'brand', 'city-map.jpg')
    expect(existsSync(art)).toBe(true)
    expect(statSync(art).size).toBeLessThan(1024 * 1024)
    expect(homeSource).toContain('import.meta.env.BASE_URL')
    expect(homeSource).toContain('brand/city-map.jpg')
