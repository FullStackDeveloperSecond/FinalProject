...
`BrandMark.vue` 只會載入本資料夾的檔案。要換圖，換這裡；
日常開發**不需要**先取得原始母檔。

| 檔名 | 用途 | 尺寸 | 大小 |
| --- | --- | --- | --- |
| `doselect-mark-40.webp` | 頂欄標記 1x（優先） | 40×40 | ~1.3 KB |
| `doselect-mark-80.webp` | 頂欄標記 2x（優先） | 80×80 | ~2.7 KB |
| `doselect-mark-120.webp` | 頂欄標記 3x（優先） | 120×120 | ~4.6 KB |
...
這六個檔必須與 `customer-web/public/brand/` 完全一致 —— 兩支 App 的
`import.meta.env.BASE_URL` 不同（`/admin/` 與 `/`），沒辦法共用同一份實體檔案。

**後台沒有 Hero 裝飾圖。** `donggu-hero-wave.png` 只有前台首頁會用，
不要複製到這裡；後台多一份等於白白增加 152 KB 的建置產物。
...
- **`alt` 依情境而定**：後台頂欄傳 `<BrandMark decorative />`，圖片用空 `alt`，
...
2. 重新產生六個衍生檔。專案沒有安裝任何影像處理套件，目前是用既有的
   ...
   `toDataURL('image/png')`。
3. 本資料夾與 `customer-web/public/brand/` **各放一份**。
